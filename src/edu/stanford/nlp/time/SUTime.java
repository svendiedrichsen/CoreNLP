...
import edu.stanford.nlp.time.temporal.Duration;
...
public class SUTime  {
 ...
 /*   MILLIS(TimexType.TIME, TimeUnit.MILLIS),
    ...
    public Temporal createTemporal(int n) {
      ...
      if (t != null) {
        t.setStandardTemporalType(this);
      }
      return t;
    }
   ...
   */
  public static Time parseDateTime(String dateStr, boolean allowPartial)
  {
    if (dateStr == null) return null;

    Optional<java.time.Instant> refInstant = parseInstant(dateStr, Optional.empty());
    if (refInstant.isPresent()) {
      return new GroundedTime(new Instant(refInstant.get().toEpochMilli()));
    }
