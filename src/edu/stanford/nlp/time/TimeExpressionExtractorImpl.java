...
import edu.stanford.nlp.time.temporal.PartialTime;
import edu.stanford.nlp.time.temporal.Temporal;
import edu.stanford.nlp.time.temporal.Time;
...
public class TimeExpressionExtractorImpl implements TimeExpressionExtractor {
    ...
    public List<CoreMap> extractTimeExpressionCoreMaps(CoreMap annotation, CoreMap docAnnotation) {
        TimeIndex timeIndex; // initialized immediately below
        String docDate = null;
        if (docAnnotation != null) {
            timeIndex = docAnnotation.get(TimeExpression.TimeIndexAnnotation.class);
            if (timeIndex == null) {
                docAnnotation.set(TimeExpression.TimeIndexAnnotation.class, timeIndex = new TimeIndex());
            }
        ...
        if (timeIndex.getDocDate() == null && docDate != null) {
            try {
                ...
                timeIndex.setDocDate(SUTime.parseDateTime(docDate, true));
            } catch (Exception e) {
    ...
    public List<TimeExpression> extractTimeExpressions(CoreMap annotation, String refDateStr, TimeIndex timeIndex) {
        Time refDate = null;
        if (refDateStr != null) {
            try {
                // TODO: have more robust parsing of document date?  docDate may not have century....
    ...
    public List<TimeExpression> extractTimeExpressions(CoreMap annotation, Time refDate, TimeIndex timeIndex) {
        ...
        if (timeIndex.getDocDate() == null) {
            if (refDate != null) timeIndex.setDocDate(refDate);
            else if (options.searchForDocDate) {
                // there was no document date but option was set to look for document date
                timeIndex.setDocDate(findReferenceDate(timeExpressions));
            }
        }
        // Didn't have a reference date - try using cached doc date
        if (refDate == null) refDate = timeIndex.getDocDate();
    ...
    private static Time findReferenceDate(List<TimeExpression> timeExpressions) {
